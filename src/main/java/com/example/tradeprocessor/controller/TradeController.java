...
public class TradeController {
  ...
  public TradeController(
      TradeProcessorService service, ObjectMapper objectMapper, Validator validator) {
  ...
  @Operation(
      summary = "Ingest a single trade",
      description =
          "Accepts a JSON payload describing a trade and returns the canonical trade id when created")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
  ...
  public ResponseEntity<String> ingest(
      @Valid
          ...
          @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "Trade to ingest",
              required = true,
              content = @Content(schema = @Schema(implementation = InputTrade.class)))
          InputTrade trade) {
  ...
  @Operation(
      summary = "Upload a file of trades",
      description = "Accepts CSV or JSON files (or newline-delimited JSON) and ingests each record")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<Object> uploadFile(
      @Parameter(description = "File containing trades (CSV or JSON)") @RequestParam("file")
          MultipartFile file,
      @Parameter(
              description =
                  "When true, the first CSV record will be treated as header and skipped from data rows")
          @RequestParam(value = "skipHeader", required = false)
          Boolean skipHeader)
      throws IOException {
      ...
      if (isCsv) {
        ...
        for (CSVRecord record : parser) {
          ...
          try {
            ...
            parseErrors
                .computeIfAbsent(originalRow, k -> new ArrayList<>())
                .add("parse error: " + e.getMessage());
          }
        }
      } else {
        // Try streaming JSON array or newline-delimited JSON
        try {
          MappingIterator<InputTrade> it =
              objectMapper.readerFor(InputTrade.class).readValues(reader);
          int idx = 0;
          while (it.hasNext()) {
            try {
          ...
          try (InputStream in2 = file.getInputStream();
              BufferedReader br =
                  new BufferedReader(new InputStreamReader(in2, StandardCharsets.UTF_8))) {
            String line;
            int idx = 0;
  ...
  @Operation(
      summary = "Get canonical trade by id",
      description = "Returns the canonical representation of a previously ingested trade")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "OK",
            content = @Content(schema = @Schema(implementation = CanonicalTrade.class))),
        @ApiResponse(responseCode = "404", description = "Not found", content = @Content)
      })
  @GetMapping("/{id}")
  public ResponseEntity<CanonicalTrade> get(
      @Parameter(description = "Canonical id of the trade to retrieve", required = true)
          @PathVariable
          String id) {
