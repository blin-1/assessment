...
public class TradeListeners {
  ...
  public void onMessage(ConsumerRecord<String, String> record) {
    ...
    log.info(
        "Received message on topic {} partition {} tradeId={}",
        record.topic(),
        record.partition(),
        tradeId == null ? "<unknown>" : tradeId);
    ...
    try {
      ...
      if (validator != null) {
        ...
        if (!violations.isEmpty()) {
          ...
          log.warn(
              "Validation failed for incoming tradeId={} violations={}",
              input.getTradeId(),
              sb.toString());
      ...
      log.debug(
          "Processing sanitized input: tradeId={} accountNumber={}",
          sanitized.getTradeId(),
          sanitized.getAccountNumber());
      ...
      log.error(
          "Failed to process incoming message on topic {} partition {} tradeId={}",
          record.topic(),
          record.partition(),
          tradeId == null ? "<unknown>" : tradeId,
          e);
      ...
      if (node.has("trade")) {
        ...
        if (validator != null) {
          ...
          if (!violations.isEmpty()) {
            ...
            log.warn(
                "Validation failed for instruction id={} tradeId={} violations={}",
                instructionId,
                input.getTradeId(),
                sb.toString());
            return;
        ...
        var sanitized = InputSanitizer.sanitizedCopy(input);
        log.debug(
            "Instruction {} contains trade tradeId={} accountNumber={}",
            instructionId == null ? "<unknown>" : instructionId,
            sanitized.getTradeId(),
            sanitized.getAccountNumber());
        ...
        log.info(
            "Instruction received with no trade payload id={}",
            instructionId == null ? "<unknown>" : instructionId);
      }
    } catch (Exception e) {
      log.error(
          "Failed to handle instruction payload on topic {} partition {} instructionId={}",
          record.topic(),
          record.partition(),
          instructionId == null ? "<unknown>" : instructionId,
          e);
