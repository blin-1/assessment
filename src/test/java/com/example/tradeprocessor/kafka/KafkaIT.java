...
public class KafkaIT {
  ...
  public void endToEndProcessingPublishesToOutputTopic() throws Exception {
    ...
    try (AdminClient admin = AdminClient.create(adminConfig)) {
      var topics =
          List.of(new NewTopic(inputTopic, 1, (short) 1), new NewTopic(outputTopic, 1, (short) 1));
      try {
    ...
    try (var producer = new KafkaProducer<String, String>(prodProps)) {
      String payload =
          "{\"tradeId\":\"IT-1\",\"accountNumber\":\"1234567890\",\"accountName\":\"Test User\",\"amount\":100.0,\"currency\":\"USD\"}";
